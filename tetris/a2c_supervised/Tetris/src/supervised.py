...
class Tetris:
    ...
    def __init__(self, height=20, width=10, block_size=20):
        ...
        self.rew_calc_weights = np.load("CEM_weights/best_weights_4_600000.npy")
    ...
    def get_board_heuristic(self, features):
        return np.dot(self.rew_calc_weights, features)

    def get_auxillary_info(self):
        ...
        for i in range(num_rotations):
            ...
            for x in range(valid_xs + 1):
                ...
                curr_idx = self.ACTION_LIST.index((x, i))
                val_actions[curr_idx] = 1
                heuristic = self.get_board_heuristic(self.get_state_properties(board))
                if heuristic > max_heu:
                    max_heu = heuristic
                    best_idx = curr_idx
        ...
        return val_actions, torch.tensor(best_idx, dtype=torch.int64)
    ...
    def step(self, action, render=False, video=None):
        ...
        return (
            self.get_custom_state(self.board),
            reward,
            self.gameover,
            self.cleared_lines
        )
